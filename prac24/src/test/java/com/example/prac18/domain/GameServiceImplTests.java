...
public class GameServiceImplTests {
    ...
    void getGames() {
        ...
        Assertions.assertEquals("vchera", gameService.getGames().get(0).getCreationDate());
    }
    ...
    void getGameById() {
        ...
        GameService gameService = new GameServiceImpl(gameRepository, gameAuthorService);
        Assertions.assertEquals("assasin",gameService.getGameById(0L).getName());
        Assertions.assertEquals("stepa",gameService.getGameById(1L).getName());
    ...
    void getGameByCreationDate() {
        ...
        Mockito.when(gameRepository.findAllByCreationDate("vchera")).thenReturn(List.of(game1));
        Mockito.when(gameRepository.findAllByCreationDate("now")).thenReturn(List.of(game2));
        GameService gameService = new GameServiceImpl(gameRepository, gameAuthorService);
        Assertions.assertEquals(1,gameService.getGameByCreationDate("vchera").size());
    ...
    void getGameByName() {
        ...
        Mockito.when(gameRepository.findAllByName("assasin")).thenReturn(List.of(game1));
        GameService gameService = new GameServiceImpl(gameRepository, gameAuthorService);
        Assertions.assertEquals(1, gameService.getGameByName("assasin").size());
        Assertions.assertEquals(0, gameService.getGameByName("assasin2").size());
    }
}
