...
public class GameAuthorServiceTests {
    ...
    void getGameAuthorByNickname() {
        ...
        Mockito.when(gameAuthorRepository.getGameAuthorsByNickname("stepa")).thenReturn(List.of(gameAuthor, gameAuthor));
        GameAuthorService gameAuthorService = new GameAuthorServiceImpl(gameAuthorRepository);
        Assertions.assertEquals(2, gameAuthorService.getGameAuthorByNickname("stepa").size());
    }
    ...
    void getGameAuthorByBirthday() {
        ...
        Mockito.when(gameAuthorRepository.getGameAuthorsByBirthday("hype")).thenReturn(List.of(gameAuthor, gameAuthor, gameAuthor));
        GameAuthorService gameAuthorService = new GameAuthorServiceImpl(gameAuthorRepository);
        Assertions.assertEquals(3, gameAuthorService.getGameAuthorByBirthday("hype").size());
    }
